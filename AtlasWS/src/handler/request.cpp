...
namespace atlas {
    ...
    void begin_response(http_request& req) {
        ...
        // Show error page if the request handler didn't begin a response either using write_head() or write()
        // and require_immediate_response is set to true (default)
    ...
    void handle_request(http_request& req) {
        ...
        server* server = req.session->server;
